
import os, sys
import numpy as np
import matplotlib; matplotlib.use("Agg")
import matplotlib.pyplot as plt
from scipy.signal import welch

sys.path.insert(0, os.path.dirname(__file__))
from common import load_data, bandpass, extract_hr, FS, TRAIN_PATH, TEST_PATH

PLOT_DIR = "../outputs/plots"
os.makedirs(PLOT_DIR, exist_ok=True)

def savefig(name):
    plt.tight_layout()
    plt.savefig(f"{PLOT_DIR}/{name}.png", dpi=130)
    plt.close()
    print(f"  {name}.png")

#Data load
print("Loading...")
train, test = np.load(TRAIN_PATH), np.load(TEST_PATH)
X_tr, y_tr  = train[:, :1000].astype(np.float32), train[:, -1].astype(np.float32)
X_te, y_te  = test[:,  :1000].astype(np.float32), test[:,  -1].astype(np.float32)
Xc_tr       = bandpass(X_tr, 0.5, 8)
hr_tr, hr_te = extract_hr(X_tr), extract_hr(X_te)
F_tr, F_te, *_ = load_data(TRAIN_PATH, TEST_PATH)
t = np.arange(1000) / FS

print(f"  Train MAP: {y_tr.mean():.1f}±{y_tr.std():.1f}  |  Test MAP: {y_te.mean():.1f}±{y_te.std():.1f}")

# MAP & HR distribution Plot
fig, axes = plt.subplots(1, 3, figsize=(15, 4))
axes[0].hist(y_tr, bins=40, color="steelblue", alpha=0.7, label="Train")
axes[0].hist(y_te, bins=40, color="tomato",    alpha=0.7, label="Test")
axes[0].axvline(y_tr.mean(), color="blue", ls="--", label=f"Train mean={y_tr.mean():.1f}")
axes[0].axvline(y_te.mean(), color="red",  ls="--", label=f"Test mean={y_te.mean():.1f}")
axes[0].set(title=f"MAP Distribution (shift={abs(y_tr.mean()-y_te.mean()):.1f} mmHg)",
            xlabel="MAP (mmHg)"); axes[0].legend(fontsize=7)

axes[1].hist(hr_tr, bins=40, color="steelblue", alpha=0.7, label="Train")
axes[1].hist(hr_te, bins=40, color="tomato",    alpha=0.7, label="Test")
axes[1].set(title="HR Distribution", xlabel="HR (bpm)"); axes[1].legend(fontsize=7)

axes[2].scatter(hr_tr, y_tr, s=5, alpha=0.3, color="steelblue", label="Train")
axes[2].scatter(hr_te, y_te, s=5, alpha=0.3, color="tomato",    label="Test")
axes[2].set(title="HR vs MAP", xlabel="HR (bpm)", ylabel="MAP (mmHg)")
axes[2].legend(fontsize=7)
fig.suptitle("Label & HR Distributions", fontweight="bold")
savefig("eda_1_distributions")

# Raw signals- low/mid/high MAP Plot
lo = np.where(y_tr < np.percentile(y_tr, 15))[0][:2]
mi = np.where((y_tr > y_tr.mean()-4) & (y_tr < y_tr.mean()+4))[0][:2]
hi = np.where(y_tr > np.percentile(y_tr, 85))[0][:2]

fig, axes = plt.subplots(2, 3, figsize=(15, 6))
for col, (idxs, label, color) in enumerate([(lo,"Low","steelblue"),(mi,"Mid","seagreen"),(hi,"High","tomato")]):
    for row, i in enumerate(idxs):
        axes[row][col].plot(t, X_tr[i], color=color, lw=0.8)
        axes[row][col].set_title(f"{label} MAP={y_tr[i]:.1f}", fontsize=8)
        axes[row][col].tick_params(labelsize=7)
fig.suptitle("Raw PPG Signals — Low / Mid / High MAP", fontweight="bold")
savefig("eda_2_signals")

# Signal decomposition Plot
Xm_tr = bandpass(X_tr, 8, 20)
fig, axes = plt.subplots(3, 3, figsize=(15, 7))
for row, i in enumerate([lo[0], mi[0], hi[0]]):
    for ax, sig, title, color in zip(axes[row],
        [X_tr[i], Xc_tr[i], Xm_tr[i]],
        [f"Raw MAP={y_tr[i]:.1f}", "Cardiac (0.5–8 Hz)", "Motion (8–20 Hz)"],
        ["gray", "steelblue", "tomato"]):
        ax.plot(t, sig, color=color, lw=0.8)
        ax.set_title(title, fontsize=8); ax.tick_params(labelsize=7)
fig.suptitle("Signal Decomposition: Raw → Cardiac → Motion", fontweight="bold")
savefig("eda_3_decomposition")

#Feature MAP correlation & shift Plot
corr  = np.array([np.corrcoef(F_tr[:, i], y_tr)[0, 1] for i in range(F_tr.shape[1])])
top   = np.argsort(np.abs(corr))[-15:][::-1]
shift = np.abs(F_te.mean(0) - F_tr.mean(0)) / (np.abs(F_tr.mean(0)) + 1e-8)

fig, axes = plt.subplots(1, 2, figsize=(14, 5))
axes[0].barh([f"F{i}" for i in top], corr[top],
             color=["steelblue" if v > 0 else "tomato" for v in corr[top]])
axes[0].axvline(0, color="black", lw=0.8)
axes[0].set(title="Top 15 Feature Correlations with MAP", xlabel="Pearson r")

axes[1].bar(range(len(shift)), shift, color="tomato", alpha=0.8)
axes[1].axhline(shift.mean(), color="navy", ls="--", label=f"Mean={shift.mean():.2f}")
axes[1].set(title="Feature Distribution Shift (Train→Test)",
            xlabel="Feature Index", ylabel="|μ_te−μ_tr|/|μ_tr|")
axes[1].legend()
fig.suptitle("Feature Analysis", fontweight="bold")
savefig("eda_4_features")

#PSD Plot
psd_tr = np.mean([welch(s, fs=FS, nperseg=256)[1] for s in X_tr[:200]], axis=0)
psd_te = np.mean([welch(s, fs=FS, nperseg=256)[1] for s in X_te[:200]], axis=0)
freqs  = welch(X_tr[0], fs=FS, nperseg=256)[0]

fig, ax = plt.subplots(figsize=(10, 4))
ax.semilogy(freqs, psd_tr, color="steelblue", lw=1.5, label="Train")
ax.semilogy(freqs, psd_te, color="tomato",    lw=1.5, ls="--", label="Test")
ax.axvspan(0.5, 8,  alpha=0.12, color="green",  label="Cardiac band")
ax.axvspan(8,  20,  alpha=0.12, color="orange", label="Motion band")
ax.set(title="Average PSD — Train vs Test", xlabel="Frequency (Hz)",
       ylabel="PSD (log)", xlim=(0, 25))
ax.legend()
savefig("eda_5_psd")

print("EDA done.")


def run():
    import os
    import numpy as np
    import scipy.stats
    import matplotlib.pyplot as plt
    from common import load_data

    os.makedirs("../outputs/plots", exist_ok=True)
    
    X_train, y_train, X_test, y_test, _, _ = load_data(
        "../data/dataset_train.npy",
        "../data/dataset_test.npy"
    )
#STD Distribution
    tr_std = X_train.std(axis=1)
    te_std = X_test.std(axis=1)

    plt.figure(figsize=(8,4))
    plt.hist(tr_std, bins=50, alpha=0.6, label="Train")
    plt.hist(te_std, bins=50, alpha=0.6, label="Test")
    plt.legend()
    plt.title("STD Distribution Shift")
    plt.savefig("../outputs/plots/std_shift.png")
    plt.close()

#Skew vs Distribution
    tr_skew = scipy.stats.skew(X_train, axis=1)
    te_skew = scipy.stats.skew(X_test, axis=1)

    plt.figure(figsize=(8,4))
    plt.hist(tr_skew, bins=50, alpha=0.6, label="Train")
    plt.hist(te_skew, bins=50, alpha=0.6, label="Test")
    plt.legend()
    plt.title("Skew Distribution Shift (Root Problem)")
    plt.savefig("../outputs/plots/skew_shift.png")
    plt.close()

#STD vs MAP
    plt.figure(figsize=(6,5))
    plt.scatter(tr_std[:3000], y_train[:3000], s=3, alpha=0.2, label="Train")
    plt.scatter(te_std, y_test, s=3, alpha=0.2, label="Test")
    plt.legend()
    plt.xlabel("STD")
    plt.ylabel("MAP")
    plt.title("STD vs MAP (Strong Correlation)")
    plt.savefig("../outputs/plots/std_vs_map.png")
    plt.close()

#Sample waveform
    plt.figure(figsize=(10,3))
    plt.plot(X_train[0], label=f"Train | MAP={y_train[0]:.1f}")
    plt.plot(X_test[0], label=f"Test  | MAP={y_test[0]:.1f}")
    plt.legend()
    plt.title("Waveform Comparison")
    plt.savefig("../outputs/plots/waveform.png")
    plt.close()

    print("EDA plots saved in outputs/plots/")
